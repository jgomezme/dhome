...
function newsCtrl($scope, $rootScope, $API, $storage){
   ...
   $scope.loadMore = function(page){
       ...
       if(!stop)
         $API
         .noticesss()
         .add('/building/'+$storage.get('buildings')[0].BuildingId+'?page='+page)
         .get()
         .success(function(rs){
   ...
   $scope.post = function(){
      ...
      if(!$scope.formm || !$scope.formm.Notice.Description)
      {
        $rootScope.alerta('Advertencia', 'Debes suministrar una descripcion para la publicación.');
        return false;
      }
...
function visitasCtrl($scope, $rootScope, $mdBottomSheet, $stateParams, $api, $storage, $location, $state, $API) {
  ...
  $scope.prealertar = function(){
      ...
      $API
      .visit()
      .add('/Auth?SuiteId='+ $rootScope.suite)  
      .post($scope._form)
      .success(function(rs,code){
          if(code === 500)
            $rootScope.alerta('Error', 'No se pudo prealertar la visita');

            $rootScope.alerta('Visita', 'Visita notificada')
            .then(function(){
                $scope._form = null;
  ...
  $scope.loadMy = function(){
   ...
   $scope.load = function(state, all){
    ...
    var params = params || {};
        ...
        $API
       .visit()
       .add("/Building/"+ $rootScope.building + (state ? "/" + state : '') + "?page=1" + (all ? '&pageSize=150' : '') )
       .get()
       .success(function(rs){
    ...
    $scope.loadMore = function(page, state){
      ...
      $API
      .visit()
      .add("/Building/"+ $rootScope.building + "/" + state + "?page=" + page )
      .get()
      .success(function(rs){
...
function correspondenceCtrl($scope, $rootScope, $API, $storage, $mdBottomSheet, $state, $stateParams){
 ...
 $rootScope.deliver = function(){
  ...
  $scope.load = function(state, all){
      ...
      $API
      .correspondencee()
      .add('/building/' + $rootScope.building + '/' + $scope.state || 'All' + (all ? '?page=1&pageSize=150' : ''))
      .get()
   ...
   //  \"All\", \"Pendings\", \"Delivereds\",
     ...
     $scope.loadMore = function(page){
      ...
      $API
      .correspondencee()
      .add('/building/' + $rootScope.building + '/' + $scope.state || 'All' + '?page='+page)
      .get()
      .success(function(rs){
...
function mainCtrl($scope, $rootScope, $window, $mdDialog, $mdSidenav, $api, $mdMedia, $mdBottomSheet, $state, $API, $storage, $location, $mdToast){
 ...
 $rootScope.toast = function(message, close){
   ...
   function toastCtrl($scope, $mdToast){
    ...
    $mdToast.show({
      ...
      function previewPhoto(n){
           ...
           var blob = typeof n != "object" ? n : window.URL.createObjectURL(n);                  
  ...
  $rootScope.stats = function(){
     ...
     $API
     .correspondenceStats($storage.get('buildings')[0].BuildingId)
     .get()
     .success(function(rs){
         $rootScope.cstats = rs;
         console.log(rs, 'correspondence')
